...
[English](README.md) · [中文](#) · [更新日志](CHANGELOG.md) · [架构](docs/ARCHITECTURE.md)

---

## 🎯 这是什么

**opsgate** 是给 DevOps 团队 + AI Agent 一起用的 **SSH 机器管理中台**,解决 3 个最常见的痛点:

1. **凭证散落** — SSH 用户名密码/私钥散落在各 AI Agent 的 prompt、IM 聊天记录、个人 wiki 里
2. **权限失控** — 谁能连哪台机器、执行什么命令,粒度太粗或根本没有
...
| 🔌 **REST API** | 自己写代码集成、构建自己的 dashboard | 平台开发 |
...
- **monorepo (npm workspaces)** — `@opsgate/server` + `@opsgate/cli` + `@opsgate/web` 三个子包
...
npm install              # monorepo,hoist 到根 node_modules
npm run build            # 构建 server / cli / web
...
Agent 就能调 8 个工具:`execute-command` / `batch-execute-command` / `upload` / `download` / `get-server-status` / `search-files` / `query-audit-logs` / `list-servers`。
...
| [README_CN.md](README_CN.md) | 中文版 README(本文) |
