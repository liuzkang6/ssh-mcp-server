...
async function request(
  ...
  if (!res.ok) {
    const errObj = data as { code?: string; message?: string } | null;
...
function printData(data: unknown, format: OutputFormat): void {
  if (format === "json") {
...
// ============== Server list resolver (for --group / --tag) ==============
...
async function listServers(ctx: RequestCtx): Promise<ServerListItem[]> {
  return (await request("GET", "/api/v1/servers", undefined, ctx)) as ServerListItem[];
}

function filterServers(
  servers: ServerListItem[],
  filter: { group?: string; tag?: string; id?: string; name?: string },
): ServerListItem[] {
  return servers.filter((s) => {
    if (filter.id && s.id !== filter.id && s.name !== filter.id) return false;
...
async function cmdConfigUnset(key: string): Promise<void> {
  const cfg = loadConfig();
...
async function cmdPing(ctx: RequestCtx, format: OutputFormat): Promise<void> {
  const data = await request("GET", "/api/v1/health", undefined, ctx);
  printData(data, format);
}

async function cmdWhoami(ctx: RequestCtx, format: OutputFormat): Promise<void> {
...
async function cmdServerCreate(
  ctx: RequestCtx,
  format: OutputFormat,
...
async function cmdServerUpdate(
  ...
  for (const [k, v] of Object.entries(opts)) {
    ...
    if (k === "tags") body.tags = String(v).split(",").map((s) => s.trim());
    else if (k === "port") body[k] = Number(v);
    else body[k] = v;
  }
...
async function cmdServerDelete(ctx: RequestCtx, id: string): Promise<void> {
  await request("DELETE", `/api/v1/servers/${encodeURIComponent(id)}`, undefined, ctx);
  console.log(chalk.green("✓ Deleted"), id);
}

async function cmdAgentList(ctx: RequestCtx, format: OutputFormat): Promise<void> {
  const data = await request("GET", "/api/v1/operators", undefined, ctx);
  printData(data, format);
}

async function cmdAgentGet(
  ctx: RequestCtx,
  format: OutputFormat,
  name: string,
): Promise<void> {
...
async function cmdAgentCreate(
  ...
  if (opts.scopes) body.scopes = String(opts.scopes).split(",").map((s) => s.trim());
  if (opts.serverPermissions) body.serverPermissions = String(opts.serverPermissions).split(",");
  const data = await request("POST", "/api/v1/operators", body, ctx);
  printData(data, format);
}

async function cmdAgentRotateKey(
  ctx: RequestCtx,
  format: OutputFormat,
  name: string,
): Promise<void> {
  const data = await request(
    "POST",
    `/api/v1/operators/${encodeURIComponent(name)}/rotate-key`,
    undefined,
    ctx,
  );
  printData(data, format);
...
async function cmdAgentDelete(ctx: RequestCtx, name: string): Promise<void> {
...
async function cmdAuditList(
  ...
  printData((data as { logs?: unknown[] }).logs ?? data, format);
}

async function cmdExec(
  ctx: RequestCtx,
  format: OutputFormat,
  args: { server: string; command: string; directory?: string; timeoutMs?: number },
): Promise<void> {
  const servers = await listServers(ctx);
  const target = filterServers(servers, { id: args.server });
  if (target.length === 0) throw new Error(`Server not found or no access: ${args.server}`);
  ...
  if (args.timeoutMs) body.timeoutMs = args.timeoutMs;
  const data = (await request(
    "POST",
    `/api/v1/servers/${encodeURIComponent(server.id)}/exec`,
    body,
    ctx,
  )) as {
    exitCode: number;
...
async function cmdBatch(
  ctx: RequestCtx,
  format: OutputFormat,
  args: { command: string; group?: string; tag?: string; parallel: number; failFast: boolean; dryRun: boolean },
): Promise<void> {
  const servers = await listServers(ctx);
  ...
  if (targets.length === 0) throw new Error("No servers matched --group/--tag");
  if (args.dryRun) {
  ...
  const runOne = async (s: ServerListItem) => {
    const start = Date.now();
    try {
      const r = (await request(
        "POST",
        `/api/v1/servers/${encodeURIComponent(s.id)}/exec`,
        { command: args.command },
        ctx,
      )) as { exitCode: number; stdout: string; durationMs: number };
      results.push({
        server: s.name,
        exitCode: r.exitCode,
      ...
      if (args.failFast && r.exitCode !== 0) throw new Error(`${s.name} exit ${r.exitCode}`);
    } catch (e) {
      results.push({
        server: s.name,
        exitCode: -1,
...
async function cmdScp(
  ctx: RequestCtx,
  format: OutputFormat,
  args: { direction: "upload" | "download"; local: string; remote: string },
): Promise<void> {
  // remote 形式 "<server>:<path>"
  const colonIdx = args.remote.indexOf(":");
  if (colonIdx < 0) throw new Error("remote must be '<server>:<path>'");
  ...
  if (target.length === 0) throw new Error(`Server not found or no access: ${serverRef}`);
  const server = target[0];
  ...
  const data = (await request("POST", path, body, ctx)) as {
    success: boolean;
...
function cmdTerminal(ctx: RequestCtx, server: string): void {
...
async function cmdStatus(
  ...
  const active = (await request(
    "GET",
    `/api/v1/servers/${encodeURIComponent(server)}/active-sessions`,
    undefined,
    ctx,
  )) as { count: number; sessions: unknown[] };
  const health = (await request("GET", "/api/v1/health", undefined, ctx)) as Record<string, unknown>;
...
async function cmdSearch(
  ctx: RequestCtx,
  format: OutputFormat,
  args: { servers: string[]; pattern: string; path?: string; parallel: number },
): Promise<void> {
...
function cmdCompletion(shell: string): void {
  let script = "";
  if (shell === "bash") script = bashCompletion();
  else if (shell === "zsh") script = zshCompletion();
  else if (shell === "fish") script = fishCompletion();
  else {
...
export async function main(argv: string[]): Promise<void> {
  ...
  server
    ...
    .action(async (opts) => {
      const ctx = makeCtx(program);
      await cmdServerCreate(ctx, formatOf(program), opts as Record<string, string>);
    });
  server
    .command("update <id>")
    .description("更新服务器")
    ...
    .action(async (id: string, opts) => {
      const ctx = makeCtx(program);
      await cmdServerUpdate(ctx, formatOf(program), id, opts as Record<string, string>);
    });
  server
    .command("delete <id>")
    .alias("rm")
  ...
  agent
    ...
    .action(async (opts) => {
      const ctx = makeCtx(program);
      await cmdAgentCreate(ctx, formatOf(program), opts as Record<string, string>);
    });
  agent
  ...
  program
    ...
    .action(async (opts) => {
      const ctx = makeCtx(program);
      await cmdAuditList(ctx, formatOf(program), opts as Record<string, string>);
    });
  ...
  program
    ...
    .action(async (server: string, commandParts: string[], opts) => {
      const ctx = makeCtx(program);
  ...
  program
    ...
    .option("--path <p>", "搜索根目录", "/")
    .option("--parallel <n>", "并发数", "5")
    .action(async (servers: string[], opts) => {
      const ctx = makeCtx(program);
...
function makeCtx(program: Command): RequestCtx {
  const opts = program.opts<{ apiKey?: string; apiBase?: string; format?: string }>();
  const cfg = loadConfig();
  const apiKey =
    process.env[ENV_API_KEY] || opts.apiKey || cfg.apiKey || "";
  const apiBase =
    process.env[ENV_API_BASE] || opts.apiBase || cfg.apiBase || DEFAULT_API_BASE;
  return { apiBase, apiKey };
}

function formatOf(program: Command): OutputFormat {
  const opts = program.opts<{ format?: string }>();
  const f = (opts.format || "text") as OutputFormat;
  if (f !== "table" && f !== "json" && f !== "text") return "text";
  return f;
}
